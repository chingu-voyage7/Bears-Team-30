...
// id (d), userid(R), challengegroupid(R), created_at (d), updated_at(d), progress(d), goal (R), status(d), start_date, days
const USER_CHALLENGES_TABLE = `CREATE TABLE IF NOT EXISTS user_challenges(id SERIAL UNIQUE NOT NULL, userid UUID REFERENCES auth(id) NOT NULL, challengegroupid INTEGER REFERENCES challenge_groups(id) NOT NULL, created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz NOT NULL DEFAULT now(), progress INTEGER NOT NULL DEFAULT 0, goal INTEGER NOT NULL, days INTEGER NOT NULL DEFAULT 0, status TEXT NOT NULL DEFAULT 'IN_PROGRESS', start_date DATE, PRIMARY KEY (userid, challengegroupid))`;
