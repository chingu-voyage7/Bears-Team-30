  ...
  if (progressPercent >= 100) {
    return (
      <div>
        ...
        <p>{completed[Math.floor(Math.random() * completed.length)]}</p>
      </div>
    );
  } else if (progressPercent < 100 && progressPercent >= 80) {
    return <p>{almostThere[Math.floor(Math.random() * almostThere.length)]}</p>;
    ...
    return (
      <p>{makingProgress[Math.floor(Math.random() * makingProgress.length)]}</p>
    );
